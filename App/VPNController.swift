    ...
    static func message(for error: Error?) -> String {
        ...
        if nsError.domain == TunnelStartupError.errorDomain,
           let stage = TunnelStartupError(rawValue: nsError.code) {
            ...
            case .egressInterfaceSelection:
                return "Не удалось определить физический интернет-интерфейс iPhone (этап 7). Переключи Wi-Fi/мобильную сеть и попробуй снова. Секретные параметры скрыты."
            }
        }
